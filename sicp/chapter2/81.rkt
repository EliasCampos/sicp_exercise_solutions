...
(define (pow x y) (apply-generic 'pow x y))
...
(define (install-scheme-number-package)
  ...
  (put 'pow '(scheme-number scheme-number)
...
(test-case
 "General operations must work with both complex and primitive numbers."
 (check-equal? (add (make-complex-from-real-imag 2 3)
                    10)
             (make-complex-from-real-imag (make-scheme-number 12) 3))
 (check-equal? (sub 5
                    (make-complex-from-real-imag 6 -2))
               (make-complex-from-real-imag -1 2))
 (check-equal? (mul 0.5
                    (make-complex-from-mag-ang 7 0))
               (make-complex-from-mag-ang 3.5 0)))

(test-case
 "(pow x y) is not defined for complex numbers and must cause an error."
 (check-exn
  #rx"No method for these types"
  (lambda () (pow (make-complex-from-real-imag 2.718 0)
                  (make-complex-from-real-imag 0 3.14)))))
