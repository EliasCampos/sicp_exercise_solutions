...
(define (get-coercion type1 type2)
  (table-get *coercion-table* (list type1 type2)))

(define (put-coercion type1 type2 op)
  (set! *coercion-table*
        (table-put *coercion-table* (list type1 type2) op)))
  ...
  (define (coercion type-tags)
    ...
    (define (try-coercion left right)
      (if (null? right)
          ...
          (let* ([type-tag (car right)]
                 [rest (cdr right)]
                 [same-type-tags (duplicate type-tag)]
                 [proc (get operation same-type-tags)]
                 [ops (map (lambda (t) (get-coer t type-tag)) type-tags)])
            (if (and proc (all? ops false))
                (apply-procedure proc (map (lambda (t1->t2 arg) (t1->t2 arg)) ops arguments))
...
(define (rational-numer r)
  ((get 'numer 'rational) (contents r)))
(define (rational-denom r)
...
(define (rational->scheme-number r)
  (make-scheme-number (/ (rational-numer r)
                         (rational-denom r))))
(put-coercion 'rational
              'scheme-number
              rational->scheme-number)
(define (rational->complex r)
  (scheme-number->complex (rational->scheme-number r)))
(put-coercion 'rational
              'complex
              rational->complex)
...
(test-case
 ...
 (check-equal? (biggest (make-complex-from-real-imag 2 3)
                        10
                        (make-rational 7 4))
               (make-complex-from-real-imag 10 0))
 (check-equal? (biggest (make-rational 5 4)
                        (make-scheme-number 1.5)
                        0.5)
               (make-scheme-number 1.5))
 (check-equal? (add (make-complex-from-mag-ang 10 0) 2)
               (make-complex-from-real-imag 12 0)))